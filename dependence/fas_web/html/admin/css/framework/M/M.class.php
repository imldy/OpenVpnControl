...
class FeedData{
	...
	function getContent($vo){
		...
		if(isset($getData['pic']) && !empty($getData['pic']) && trim($getData['pic'])!= ''){
				...
				;
				if($vo['action'] == 'zhuan'){	
				$html .= '<a href="javascript:void(0)" onclick="" id="pic-box-'.$vo['data']['id'].'" title="点击查看"><ul libs="feed-pic-z">';
					foreach($picArr as $vp){
						$html .= '<li><img src="'.$vp.'" alt="加载中..."></li>';
					}
					$html .= '</ul></a><div libs="clear"></div>';
				}else{
					$html .= '<a href="javascript:void(0)" onclick="javascript:setImages = '.$vo['data']['id'].'" id="pic-box-'.$vo['data']['id'].'" title="点击查看"><ul libs="feed-pic">';
					if($n < 1 or $n >3){
						$n = '';
					}
					foreach($picArr as $vp){
						$html .= '<li libs="col'.$n.'"><img src="'.$vp.'" alt="加载中..."></li>';
					}
					$html .= '</ul></a><div libs="clear"></div>';
				}
			}
